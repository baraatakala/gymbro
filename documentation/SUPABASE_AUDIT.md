...
Do **not** pass `project_id` ΓÇö the MCP URL is already scoped to trail.
...
**Integrity:** `total_volume_kg` matches `sum(weight ├ù reps)` on sets. PR trigger `trg_update_pr_on_set` active. Session user trigger `trg_set_workout_session_user_id` active.
...
- **Anonymous policies** ΓÇö app uses anonymous auth; user tables still scoped with `auth.uid() = user_id`.
- **SECURITY DEFINER RPC** ΓÇö `REVOKE EXECUTE` from `anon` + `authenticated`; triggers only.
- **Leaked password protection** ΓÇö enable in Dashboard if you add email/password later.

## Schema reference

See [DATA_MODEL.md](./DATA_MODEL.md) for FKs, constraints, RLS, triggers, and indexing review.

## Client Γåö DB alignment
